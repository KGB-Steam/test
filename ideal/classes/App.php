...
class App extends Singleton{
	...
	public function __construct(){
		...
		$default_config = include CONFIG . 'main.php';
		
        $this->config = new Registry($default_config);

        $db_name = 'db_' . $this->config->db['dbtype'];

       	include DATABASES . $db_name . '.php';
    ...
    function start(){
        ...
        $controller = app::gi(ucfirst($this->uri->controller) . 'Controller');
		ob_start();
		...
		if (($this->uri->{'controller'} == 'game') && ($this->uri->{'action'} == 'answer')) {
			$controller->renderGame($content);
		} else {
